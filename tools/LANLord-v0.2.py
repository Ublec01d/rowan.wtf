...
def nmap_discovery(subnet):
    if is_deep_scan:
        args = "-A -T4 -p- -sV -v --script=vuln"
    else:
        args = "-sn -PR"

    cmd = ["nmap"] + args.split() + [subnet]
    window.after(0, lambda: output_text.insert(tk.END, f"🛠 Running: {' '.join(cmd)}\n"))
    ...
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            if stop_scan_flag:
                process.terminate()
                window.after(0, lambda: output_text.insert(tk.END, "⛔ Nmap scan aborted.\n"))
                break

            window.after(0, lambda l=line: output_text.insert(tk.END, l))
            window.after(0, output_text.see, tk.END)

            # Optional parsing for result-building (if you still want that)
        ...
        process.wait()
        window.after(0, lambda: output_text.insert(tk.END, f"✅ Finished scanning {subnet}\n"))

    except FileNotFoundError:
        window.after(0, lambda: output_text.insert(tk.END, "❌ Nmap not found. Add to PATH?\n"))
        ...
        if results:
            ...
            for entry in sorted(unique_entries):
                if len(entry) == 4:
                    ...
                    port_str = "<br>".join(ports) if ports else "-"
                else:
                    ip, mac, name = entry
                    port_str = "-"
...
# --- GUI Setup ---
window = tk.Tk()
window.title("LANLord v0.2 - RFC1918 Edition")
...
*Beware! Deep Scan is very aggresive!
 Might trigger security systems (firewalls, IPS)
